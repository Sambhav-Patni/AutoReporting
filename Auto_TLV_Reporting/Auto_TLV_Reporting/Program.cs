...
namespace Auto_TLV_Reporting
{
    class Program
    {
        ...
        static string cocd="XX";
        static int TSDays = 15;
        static string dateTo = DateTime.Now.Date.ToString("1-MMM-yyyy");//"12-JAN-2015";
        static string dateFrom = DateTime.Now.Date.Subtract(TimeSpan.FromDays(TSDays)).ToString("1-MMM-yyyy");//"05-JAN-2015";        
        static string path = "book.xls";
        static string Output = "<>_AllStates_PlaceHolder.xls";
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                config = File.ReadAllLines("Config.sam");
                dateFrom = Convert.ToDateTime(config[1]).ToString("dd-MMM-yyyy");
                dateTo = Convert.ToDateTime(config[2]).ToString("dd-MMM-yyyy");
            }
            catch { }            
            Output = Output.Replace("PlaceHolder", Convert.ToDateTime(dateFrom).Day + "-" + (Convert.ToDateTime(dateTo).Subtract(TimeSpan.FromDays(1)).Day) + "_" + Months[Convert.ToDateTime(dateFrom).Month - 1] + "_" + Convert.ToDateTime(dateFrom).Year);
            ...
            try
            {
                config = File.ReadAllLines("Config.sam");
                cocd = config[0];
                config = File.ReadAllLines("DBConfig.sam");
                ...
                if (upload == true)
                {
                    ...
                    {
                        File.WriteAllText("SPErrorLog.txt", "Error: \n" + ex.Message + "\n" + ex.StackTrace);
                    }
                }

                mail(false);
            }
            //catch (Exception ex) { Console.WriteLine(ex.Message); mail(true, ex.Message); }
            catch (Exception ex)
            {
        ...
        public static void vba(string type)
        {
            ...
            var codeText = File.ReadAllText("Process.vb");
        ...
        {
            string QueryDistinct = "select org_cd as \"USER ID\",upper(org_name)as \"USER NAME\" from epd_org  where upper(org_cd) in ( PlaceHolder )  and org_type_name ='USER'  and co_cd in ('"+cocd+"')  order by org_cd";
        ...
        static void Finalize()
        {
            ...
            if (!File.Exists(Output))
            {
                ...
                xlWB.SaveAs(Directory.GetCurrentDirectory() + "\\" + Output, Microsoft.Office.Interop.Excel.XlFileFormat.xlExcel8, Missing.Value, Missing.Value, Missing.Value, Missing.Value, Microsoft.Office.Interop.Excel.XlSaveAsAccessMode.xlNoChange, Missing.Value, Missing.Value, Missing.Value, Missing.Value, Missing.Value);
                xlWB.Close();
                xlApp.Quit();
            }
            //DataSet ds = new DataSet();
        ...
        static void mail(bool error, string msg = "")
        {
            ...
            message.Subject = "TLV Report for "+cocd+"-All States";
            if (error == true)
                message.Body = "Error Occured<br/><br/>" + msg + "<br/><br/>Thanks,\n<br/>Sambhav Patni<br/><br/>Auto Generated On " + DateTime.Now.ToString();
            else
            {
                message.Body = "Hi,\n\n<br/><br/>Please find the TLV Report “Report for "+cocd+"-All States at below mentioned link:”\n<br/>";
